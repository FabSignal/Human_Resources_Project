// Configuración base
const API_BASE = "https://pcos-tracker-api.onrender.com";
...
// Ejemplo: Obtener historial de síntomas
async function getSymptomHistory(userId) {
  try {
    const response = await fetch(`${API_BASE}/api/symptoms/history/${userId}`);
    const data = await response.json();
    displaySymptomHistory(data); // Función que mostrará los datos
  } catch (error) {
    console.error("Fetch error:", error);
  }
}

// Mostrar historial de síntomas
function displaySymptomHistory(data) {
  ...
  data.forEach((entry) => {
    ...
    entryEl.className = "symptom-item";
    entryEl.innerHTML = `
      <div>
        <strong>${new Date(entry.date).toLocaleDateString()}</strong>
        <div>${entry.symptoms.join(", ")}</div>
      </div>
      <div class="severity">${"★".repeat(entry.severity)}</div>
    `;
...
function displayHealthInsights(data) {
  ...
  document.getElementById("period-prediction").innerHTML = `
    <h3>Próximo período</h3>
    <div class="date">${new Date(data.nextPeriod).toLocaleDateString()}</div>
    <p>${data.cycleInfo}</p>
  `;
}
